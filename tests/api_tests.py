...
class APITestRunner:
    ...
    def run_single_test(self, test_config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == expected_status:
                ...
                print(
                    f"  ❌ FAILED - {test_config['name']} (Expected {expected_status}, got {response.status_code})"
                )
    ...
    def save_results_locally(self) -> None:
        """Save test results to a local JSON file"""
        try:
            ...
            # Create complete summary for saving
            summary = {
                "test_suite_id": self.test_id,
                "timestamp": datetime.now(timezone.utc).isoformat(),
                "total_tests": len(self.results),
    ...
    def send_results_to_service_bus(self, summary: Dict[str, Any]):
        ...
        try:
            print(
                f"📤 {self.runner_label} → Azure Service Bus (Messaging) - Sending results..."
            )
            ...
            with ServiceBusClient.from_connection_string(
                ...
                print(
                    f"✅ {self.runner_label} → Azure Service Bus (Messaging) - Results sent successfully!"
                )
                print(
                    "🔄 Next: Azure Service Bus (Messaging) → Azure Function (Results Processor)"
                )
            ...
            if "timed out" in str(e).lower() or "amqp:socket-error" in str(e):
                print(
                    f"💡 Network connectivity issue detected from {self.runner_label}"
                )
                if self.environment == "local-machine":
                    print(
                        "   This is expected on local machine due to firewall/network restrictions"
                    )
...
def main():
    ...
    runner.save_results_locally()

    # Then send to Service Bus
    runner.send_results_to_service_bus(summary)

    # Remove the duplicate local save logic that was here
    ...
    if summary["failed_tests"] > 0:
        print(
            f"❌ Exiting with failure code - {summary['failed_tests']} test(s) failed"
        )
        exit(1)
    else:
